...
def process_video(input_video_path, output_video_path, is_draw_zone_control, is_draw_pass_lines):
    ...
    while cap.isOpened():
        ...
        # Step 2: classify based on jersey color
        if len(players) > 0:
        ...
        for i, track_id in enumerate(players.tracker_id):
            if i < len(teams):
                color_history.setdefault(track_id, []).append(teams[i])
        ...
        if is_draw_pass_lines:
            if ball_holder and ball_holder in team_assignments:
                ...
                annotated = draw_pass_lines(annotated, players, available_passes, TEAM_COLORS[team_assignments[ball_holder]], ball_holder)
